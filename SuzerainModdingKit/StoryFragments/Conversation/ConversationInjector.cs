...
internal static class ConversationInjector
{
    ...
    {
        DialogueEntry parent = conversation.GetDialogueEntry(node.ParentID);
        if (parent == null)
        {
            Melon<Core>.Logger.Error("Failed to inject ConversationNode: " +
                $"Parent node with ID {node.ParentID.ToString(CultureInfo.InvariantCulture)} " +
                $"not found in conversation '{conversation.Title}'.");
            return;
        }
        ...
        {
            newEntry.ActorID = node.SpeakerID ?? 0;
            // The conversant should be the player.
            newEntry.ConversantID = conversation.ActorID;
        }

        Link nextLink = new(
            conversation.id, newEntry.id,
            conversation.id, node.NextID);
